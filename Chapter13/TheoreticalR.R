...
DrawTails <- function(isRScale=F ) {
  ...
  #   xLower <- seq(from=-.2, to=zCritLower, len=length)
...
zTicksMajor <- atanh(rhoTicksMajor)
zTicksMinor <- atanh(rhoTicksMinor)
zTickMajorLabels <- c(round(zTicksMajor[1:2], 2), "", round(zTicksMajor[4:5], 2))

oldPar <- par( mfrow=c(2,1), mgp=c(.5, 0, 0), tcl=.2, mar=c(1.65, 1.15, .5, .2))

###Z
plot(NA, xlim=c(-1.16,1.16), ylim=yRange, fg=gray(.9), bty="o", yaxs="i", xaxs="i", tcl=-.5,
     yaxt="n",xaxt="n",
     col.lab=gray(.4), cex.lab=.8,
     main="",
     xlab=expression(italic(z)), ylab="Density")
axis(1, at=zTicksMajor, labels=zTickMajorLabels, cex.axis=.7, col=gray(.5), col.axis=gray(.5),tcl=0)
axis(1, at=zTicksMinor, labels=c("","","","","","","","","","",""), col=gray(.5), col.axis=gray(.5),tcl=-.15)
...
DrawTails( isRScale=F)
lines(zPossible, priorZ, col=colorPrior)
# lines(zPossible, likelihoodZ, col=colorLikelihood)
# lines(zPossible, posteriorZ, col=colorPosterior)

DrawLine(at=zPrior, label=expression(italic(z)[Prior]), color=colorPrior, labelLine=-.75, lineType=3, alignPosition=1)
...
DrawLine(at=rPrior, label=expression(italic(r)[Prior]), color=colorPrior, labelLine=-.75, lineType=3, alignPosition=1)

par(oldPar)

#c(sum(priorZ), sum(likelihoodZ)) * intervalWidth
#c(sum(priorR), sum(likelihoodR)) * intervalWidth